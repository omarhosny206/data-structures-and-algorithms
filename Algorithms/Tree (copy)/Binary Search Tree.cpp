...
private:
    ...
    void getMaxNumberLength(TreeNode *root, int &maxNumberLength)
    {
        if (root == nullptr)
            return;
        string number = to_string(root->val);
        int length = number.length();
        maxNumberLength = max(maxNumberLength, length);
        getMaxNumberLength(root->left, maxNumberLength);
        getMaxNumberLength(root->right, maxNumberLength);
    }
...
public:
    ...
    void Print()
    {
        ...
        vector<vector<string>> nodes(height, vector<string>(pow(2, height) - 1, string(maxNumberLength, ' ')));
        Construct(root, nodes, 0, 0, nodes[0].size() - 1);

        for (vector<string> &level : nodes)
        {
            for (string &node : level)
                cout << string(maxNumberLength - node.length(), ' ') + node;
    ...
    {
        int maxNumberLength = INT_MIN;
        getMaxNumberLength(root, maxNumberLength);
