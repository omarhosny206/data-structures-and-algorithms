...
private:
    ...
    int getPredecessor2(TreeNode *root, int val)
    {
        int result = -1;
    ...
    int getSuccessor2(TreeNode *root, int val)
    {
        int result = -1;
...
public:
    ...
    int getPredecessor2(int val)
    {
        return getPredecessor2(root, val);
    }

    int getSuccessor2(int val)
    {
        return getSuccessor2(root, val);
    }

    void printPredecessor(vector<int> &nodes)
    {
        for (int &node : nodes)
            cout << "Predecessor of (" << node << ") is "
                 << "(" << getPredecessor(node) << ")" << endl;
    }

    void printSuccessor(vector<int> &nodes)
    {
        for (int &node : nodes)
            cout << "Successor of (" << node << ") is "
                 << "(" << getSuccessor(node) << ")" << endl;
    }

    void printPredecessor2(vector<int> &nodes)
    {
        for (int &node : nodes)
            cout << "Predecessor of (" << node << ") is "
                 << "(" << getPredecessor2(node) << ")" << endl;
    }

    void printSuccessor2(vector<int> &nodes)
    {
        for (int &node : nodes)
            cout << "Successor of (" << node << ") is "
                 << "(" << getSuccessor2(node) << ")" << endl;
    }

    void testPredecessorAndSuccessorFunctions(vector<int> &nodes)
    {
        for (int &node : nodes)
        {
            if ((getPredecessor(node) == getPredecessor2(node)) && (getSuccessor(node) == getSuccessor2(node)))
                cout << "Passed .." << endl;
...
int main()
{
    vector<int> nodes = {100, 130, 20, 60, 30, 50};
    BinarySearchTree *tree = new BinarySearchTree(nodes);

    cout << endl;

    tree->printPredecessor(nodes);
    cout << endl;

    tree->printPredecessor2(nodes);
    cout << endl;

    tree->printSuccessor(nodes);
    cout << endl;

    tree->printSuccessor2(nodes);
    cout << endl;

    tree->testPredecessorAndSuccessorFunctions(nodes);
    cout << endl;
}